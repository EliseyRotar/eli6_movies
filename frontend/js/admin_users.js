...
// Auth helpers use cookie-based auth; user info in localStorage only
...
async function checkAdminAccess() {
    try {
        // Validate cookie session by calling profile
...
async function onAdminLoginSubmit(e) {
    ...
    try {
        ...
        // Store user; token is httpOnly cookie
...
// Compute and render top-level stats
function updateStats(users) {
    try {
        const totalUsersEl = document.getElementById('totalUsers');
        const activeUsersEl = document.getElementById('activeUsers');
        const avgMyListEl = document.getElementById('avgMyList');
        const avgWatchHistoryEl = document.getElementById('avgWatchHistory');
        ...
        const avgList =
            total > 0
                ? Math.round(
                      (users || []).reduce(
                          (sum, u) => sum + ((u.myList && u.myList.length) || 0),
                          0
                      ) / total
                  )
                : 0;
        const avgWH =
            total > 0
                ? Math.round(
                      (users || []).reduce(
                          (sum, u) => sum + ((u.watchHistory && u.watchHistory.length) || 0),
                          0
                      ) / total
                  )
                : 0;
        ...
        if (avgMyListEl) avgMyListEl.textContent = String(avgList);
        if (avgWatchHistoryEl) avgWatchHistoryEl.textContent = String(avgWH);
    } catch (err) {
        // Don't break UI rendering if stats fail
        console.error('updateStats error:', err);
    }
}

// Modal logic used by HTML
