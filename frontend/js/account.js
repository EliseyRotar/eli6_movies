...
(function () {
  ...
  function renderAuthForms(mount) {
    ...
    // Google OAuth — backend redirects through accounts.google.com and bounces
    // back to APP_URL with the session cookie set. The `returnTo` ensures the
    // user lands back on the account page (or wherever they came from).
    var orRow = el('div');
    orRow.style.cssText = 'display:flex;align-items:center;gap:12px;margin:18px 0;color:var(--fg-muted);font-size:12px';
    orRow.innerHTML = '<div style="flex:1;height:1px;background:var(--border)"></div><span>or</span><div style="flex:1;height:1px;background:var(--border)"></div>';
    loginForm.appendChild(orRow);

    var googleBtn = el('a', 'btn btn--ghost');
    googleBtn.href = (window.API_BASE_URL || '') + '/auth/google/start?returnTo=' + encodeURIComponent(location.pathname);
    googleBtn.style.cssText = 'width:100%;display:flex;align-items:center;justify-content:center;gap:10px;padding:11px;font-size:14px;text-decoration:none';
    googleBtn.innerHTML = '<svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true"><path fill="#FFC107" d="M43.6 20.5H42V20H24v8h11.3C33.7 32.4 29.3 35.5 24 35.5c-6.4 0-11.5-5.1-11.5-11.5S17.6 12.5 24 12.5c2.9 0 5.6 1.1 7.6 2.9l5.7-5.7C33.6 6.4 29.1 4.5 24 4.5 13.2 4.5 4.5 13.2 4.5 24S13.2 43.5 24 43.5 43.5 34.8 43.5 24c0-1.2-.1-2.4-.4-3.5z"/><path fill="#FF3D00" d="M6.3 14.7l6.6 4.8C14.7 16 19 12.5 24 12.5c2.9 0 5.6 1.1 7.6 2.9l5.7-5.7C33.6 6.4 29.1 4.5 24 4.5c-7.5 0-14 4.3-17.7 10.2z"/><path fill="#4CAF50" d="M24 43.5c5 0 9.6-1.9 13.1-5l-6-5.1c-2 1.4-4.5 2.2-7.1 2.2-5.3 0-9.7-3.4-11.3-8H6.1l-1.6 1.2C8.1 38.9 15.4 43.5 24 43.5z"/><path fill="#1976D2" d="M43.6 20.5H42V20H24v8h11.3c-.8 2.3-2.3 4.3-4.2 5.4l6 5.1c-.4.4 7-5.1 7-14.5 0-1.2-.1-2.4-.5-3.5z"/></svg>Sign in with Google';
