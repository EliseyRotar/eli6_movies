...
<head>
    ...
    <link rel="stylesheet" href="css/spatial-nav.css">
    ...
    <style>
        ...
        *::-webkit-scrollbar {
        ...
        /* Fullscreen Mouse Catcher Override */
        .video-section.fullscreen #mouse-catcher {
        ...
        @media (max-width: 768px) {
            ...
            /* Reuse styles from index.css for consistency */
            .mobile-topbar {
                position: fixed;
                top: 0;
                left: 0;
            ...
            /* Hero Content - Show FIRST (above player) */
            .hero-content {
            ...
            /* Video Player - Show SECOND (below hero content) */
            .video-section {
                height: auto;
                aspect-ratio: 16/9;
                max-width: 95%;
...
<body>
    ...
    <div class="mobile-topbar" id="mobileTopbar">
        <button class="mobile-hamburger" id="openMobileSidenav">
            <span></span><span></span><span></span>
        </button>
        <span class="mobile-logo" data-i18n="nav.logo">ELI6 MOVIES</span>
        <div class="mobile-top-icons">
            <a href="account.html" class="mobile-action-btn">
                <i class="material-icons">account_circle</i>
            </a>
        </div>
    </div>

    <!-- Mobile Overlay and Side Navigation -->
    <div class="mobile-overlay" id="mobileOverlay" style="display: none"></div>
    <nav class="mobile-sidenav" id="mobileSidenav" style="display: none">
        <div class="mobile-sidenav-header">
            <span class="mobile-sidenav-title" data-i18n="nav.logo">ELI6 MOVIES</span>
            <button class="mobile-sidenav-close" id="closeMobileSidenav">&times;</button>
        </div>
        <ul class="mobile-sidenav-links">
            <li><a href="index.html" data-i18n="nav.home">Home</a></li>
            <li><a href="movies.html" data-i18n="nav.movies">Movies</a></li>
            <li><a href="tvshows.html" data-i18n="nav.tvshows">TV Shows</a></li>
            <li><a href="mylist.html" data-i18n="nav.mylist">My List</a></li>
    ...
    <nav class="mobile-bottom-nav">
        ...
        <a href="movies.html">
            ...
            <span data-i18n="nav.movies">Movies</span>
        </a>
        <a href="tvshows.html">
            <i class="material-icons">tv</i>
            <span data-i18n="nav.tvshows">TV</span>
        </a>
        <a href="mylist.html">
            <i class="material-icons">playlist_add_check</i>
            <span data-i18n="nav.mylist">My List</span>
        </a>
    ...
    <section class="hero-section">
        ...
        <div class="hero-content" style="display: block !important; visibility: visible !important; opacity: 1 !important;">
            <div class="video-title-scroll" style="display: block !important; visibility: visible !important;"><span class="video-title" id="heroTitle" data-i18n="player.loading" style="display: block !important; visibility: visible !important; opacity: 1 !important; color: #fff !important;">Loading...</span></div>
            <div class="hero-meta" style="display: flex !important; visibility: visible !important; opacity: 1 !important; justify-content: center !important;">
            ...
            <p class="hero-description" id="heroDescription" data-i18n="player.loadingContent" style="display: block !important; visibility: visible !important; opacity: 1 !important; color: #e5e5e5 !important;">Loading content information...</p>
        ...
        <div id="videoPlayer">
            <div class="video-section" id="videoSection" style="display: block; margin-top: 0;">
                <div class="video-player">
                    ...
                    <!-- Mouse Catcher: Transparent overlay to capture mouse events when controls are hidden -->
                    <div id="mouse-catcher" style="position:absolute; top:0; left:0; width:100%; height:100%; z-index:2147483647; display:none; background: transparent;"></div>
    ...
    <script>
        const TMDB_BASE_URL = 'https://streaming.ecolens.me/api/tmdb';
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            if (videoPlayerDiv && !document.getElementById('manualControls')) {
                ...
                document.getElementById('reportBrokenBtn').onclick = () => {
        ...
        async function loadContentDetails() {
            try {
                ...
                if (type === 'anime') {
                    ...
                    const response = await fetch(`${TMDB_BASE_URL}/${endpoint}/${id}?language=${lang}`);
                    if (!response.ok) {
                        const errorText = await response.text();
                    ...
                    const extIdsResp = await fetch(`${TMDB_BASE_URL}/${endpoint}/${id}/external_ids`);
                    if (extIdsResp.ok) {
        ...
        async function updateAnimeContentInfo() {
            ...
            // Set hero backdrop (for anime, use the provided image)
            const heroBackdrop = document.querySelector('.hero-backdrop');
            if (heroBackdrop && image) {
                heroBackdrop.style.backgroundImage = `url('${image}')`;
            } else if (heroBackdrop && contentData && contentData.images && contentData.images.jpg && contentData.images.jpg.large_image_url) {
                // Fallback for anime
                heroBackdrop.style.backgroundImage = `url('${contentData.images.jpg.large_image_url}')`;
            }
        ...
        async function updateContentInfo() {
            ...
            // Set backdrop image, fallback to poster if backdrop not available
            const heroBackdropEl = document.getElementById('heroBackdrop');
            if (heroBackdropEl) {
                if (backdropPath) {
                    heroBackdropEl.style.backgroundImage = `url(${backdropPath})`;
                } else if (contentData.poster_path) {
                    // Fallback to poster image if no backdrop
                    const posterPath = `https://image.tmdb.org/t/p/original${contentData.poster_path}`;
                    heroBackdropEl.style.backgroundImage = `url(${posterPath})`;
                }
            }
        ...
        const SERVER_LIST = [
            ...
            { key: 'dood', label: 'Dood' },
            { key: 'streamlare', label: 'Streamlare' }
        ];
        ...
        // Function to test iframe loading
        function testIframeLoading(url) {
        ...
        function loadVideoPlayer() {
            ...
            if (currentServer === 'vidsrc') {
                if (type === 'movie') {
                    // VidSrc uses IMDB ID for movies, fallback to TMDB ID
                    if (contentData && contentData.external_ids && contentData.external_ids.imdb_id && contentData.external_ids.imdb_id.startsWith('tt')) {
                        videoUrl = `https://vidsrc.to/embed/movie/${contentData.external_ids.imdb_id}`;
                    } else if (contentData && contentData.external_ids && contentData.external_ids.tmdb_id) {
                        videoUrl = `https://vidsrc.to/embed/movie/${contentData.external_ids.tmdb_id}`;
                    } else {
                        videoUrl = `https://vidsrc.to/embed/movie/${id}`;
                    }
                } else if (type === 'tv') {
                    // VidSrc uses IMDB ID for TV shows, fallback to TMDB ID
                    if (contentData && contentData.external_ids && contentData.external_ids.imdb_id && contentData.external_ids.imdb_id.startsWith('tt')) {
                        videoUrl = `https://vidsrc.to/embed/tv/${contentData.external_ids.imdb_id}/${currentSeasonValue}/${currentEpisodeValue}`;
                    } else if (contentData && contentData.external_ids && contentData.external_ids.tmdb_id) {
                        videoUrl = `https://vidsrc.to/embed/tv/${contentData.external_ids.tmdb_id}/${currentSeasonValue}/${currentEpisodeValue}`;
                    } else {
                        videoUrl = `https://vidsrc.to/embed/tv/${id}/${currentSeasonValue}/${currentEpisodeValue}`;
                    }
                } else {
                    videoUrl = `https://vidsrc.to/embed/movie/${id}`;
                }
                ...
                if (type === 'movie') {
                    // VIXSRC uses TMDB ID for movies
                    if (contentData && contentData.external_ids && contentData.external_ids.tmdb_id) {
                        videoUrl = `https://vixsrc.to/movie/${contentData.external_ids.tmdb_id}`;
                    } else {
                        videoUrl = `https://vixsrc.to/movie/${id}`;
                    }
                } else if (type === 'tv') {
                    // VIXSRC uses TMDB ID for TV shows
                    if (contentData && contentData.external_ids && contentData.external_ids.tmdb_id) {
                        videoUrl = `https://vixsrc.to/tv/${contentData.external_ids.tmdb_id}/${currentSeasonValue}/${currentEpisodeValue}`;
                    } else {
            ...
            } else if (currentServer === 'vidcloud') {
                // VidCloud server logic - more permissive
                if (type === 'movie') {
                    if (contentData && contentData.external_ids && contentData.external_ids.imdb_id && contentData.external_ids.imdb_id.startsWith('tt')) {
                        videoUrl = `https://vidcloud.to/embed/${contentData.external_ids.imdb_id}`;
                    } else if (contentData && contentData.external_ids && contentData.external_ids.tmdb_id) {
                        videoUrl = `https://vidcloud.to/embed/${contentData.external_ids.tmdb_id}`;
                    } else {
                        videoUrl = `https://vidcloud.to/embed/${id}`;
                    }
                } else if (type === 'tv') {
                    if (contentData && contentData.external_ids && contentData.external_ids.imdb_id && contentData.external_ids.imdb_id.startsWith('tt')) {
                        videoUrl = `https://vidcloud.to/embed/${contentData.external_ids.imdb_id}?s=${currentSeasonValue}&e=${currentEpisodeValue}`;
                    } else if (contentData && contentData.external_ids && contentData.external_ids.tmdb_id) {
                        videoUrl = `https://vidcloud.to/embed/${contentData.external_ids.tmdb_id}?s=${currentSeasonValue}&e=${currentEpisodeValue}`;
                    } else {
                        videoUrl = `https://vidcloud.to/embed/${id}?s=${currentSeasonValue}&e=${currentEpisodeValue}`;
                    }
                } else {
                    videoUrl = `https://vidcloud.to/embed/${id}`;
                }
            } else if (currentServer === 'dood') {
                // Dood server logic - more permissive
                if (type === 'movie') {
                    if (contentData && contentData.external_ids && contentData.external_ids.imdb_id && contentData.external_ids.imdb_id.startsWith('tt')) {
                        videoUrl = `https://dood.ws/e/${contentData.external_ids.imdb_id}`;
                    } else if (contentData && contentData.external_ids && contentData.external_ids.tmdb_id) {
                        videoUrl = `https://dood.ws/e/${contentData.external_ids.tmdb_id}`;
                    } else {
                        videoUrl = `https://dood.ws/e/${id}`;
                    }
                } else if (type === 'tv') {
                    if (contentData && contentData.external_ids && contentData.external_ids.imdb_id && contentData.external_ids.imdb_id.startsWith('tt')) {
                        videoUrl = `https://dood.ws/e/${contentData.external_ids.imdb_id}?s=${currentSeasonValue}&e=${currentEpisodeValue}`;
                    } else if (contentData && contentData.external_ids && contentData.external_ids.tmdb_id) {
                        videoUrl = `https://dood.ws/e/${contentData.external_ids.tmdb_id}?s=${currentSeasonValue}&e=${currentEpisodeValue}`;
                    } else {
                        videoUrl = `https://dood.ws/e/${id}?s=${currentSeasonValue}&e=${currentEpisodeValue}`;
                    }
                } else {
                    videoUrl = `https://dood.ws/e/${id}`;
                }
            } else if (currentServer === 'streamlare') {
                // Streamlare server logic - more permissive
                if (type === 'movie') {
                    if (contentData && contentData.external_ids && contentData.external_ids.imdb_id && contentData.external_ids.imdb_id.startsWith('tt')) {
                        videoUrl = `https://streamlare.to/e/${contentData.external_ids.imdb_id}`;
                    } else if (contentData && contentData.external_ids && contentData.external_ids.tmdb_id) {
                        videoUrl = `https://streamlare.to/e/${contentData.external_ids.tmdb_id}`;
                    } else {
                        videoUrl = `https://streamlare.to/e/${id}`;
                    }
                } else if (type === 'tv') {
                    if (contentData && contentData.external_ids && contentData.external_ids.imdb_id && contentData.external_ids.imdb_id.startsWith('tt')) {
                        videoUrl = `https://streamlare.to/e/${contentData.external_ids.imdb_id}?s=${currentSeasonValue}&e=${currentEpisodeValue}`;
                    } else if (contentData && contentData.external_ids && contentData.external_ids.tmdb_id) {
                        videoUrl = `https://streamlare.to/e/${contentData.external_ids.tmdb_id}?s=${currentSeasonValue}&e=${currentEpisodeValue}`;
                    } else {
                        videoUrl = `https://streamlare.to/e/${id}?s=${currentSeasonValue}&e=${currentEpisodeValue}`;
                    }
                } else {
                    videoUrl = `https://streamlare.to/e/${id}`;
                }
            ...
            console.log(`[Player] Loading video from server: ${SERVER_LIST[currentServerIndex].label}`);
            console.log(`[Player] Video URL: ${videoUrl}`);
            console.log(`[Player] Content type: ${type}, ID: ${id}`);
            console.log(`[Player] Content data:`, contentData);
            console.log(`[Player] External IDs:`, contentData?.external_ids);
            ...
            newIframe.onerror = () => {
                console.log(`[Player] Iframe error for server: ${SERVER_LIST[currentServerIndex].label}`);
            };
            newIframe.onload = () => {
                console.log(`[Player] Iframe loaded successfully for server: ${SERVER_LIST[currentServerIndex].label}`);
                errorHandled = false;
            ...
            failoverTimeout = setTimeout(() => {
                if (!errorHandled) {
                    console.log('[Player] Timeout waiting for iframe to load');
                }
            }, 8000);
        ...
        async function loadTrailers() {
            try {
                ...
                const response = await fetch(`${TMDB_BASE_URL}/${endpoint}/${id}/videos`);
                if (response.ok) {
                    const data = await response.json();
        ...
        async function loadSimilar() {
            try {
                const response = await fetch(`${TMDB_BASE_URL}/${type}/${id}/similar`);
                if (response.ok) {
                    const data = await response.json();
        ...
        async function loadSeasonEpisodes(seasonNumber) {
            try {
                ...
                const response = await fetch(`${TMDB_BASE_URL}/tv/${id}/season/${seasonNumber}`);
                if (response.ok) {
        ...
        // Restore Play button functionality: play selected server
        function playContent() {
            ...
            loadVideoPlayer();
            isVideoPlaying = true;
            // Don't auto-enter fullscreen - let user control it
        ...
        function updateOverlayControls() {
            // Server buttons - add null checks
            const serverBtnVixsrc = document.getElementById('serverBtnVixsrc');
            const serverBtnVidsrc = document.getElementById('serverBtnVidsrc');
            if (serverBtnVixsrc) serverBtnVixsrc.classList.toggle('active', currentServerIndex === 0);
            if (serverBtnVidsrc) serverBtnVidsrc.classList.toggle('active', currentServerIndex === 1);
            
            // Episode selector
            const overlayEpisodeSelector = document.getElementById('overlayEpisodeSelector');
            if (type === 'tv') {
                if (overlayEpisodeSelector) overlayEpisodeSelector.style.display = 'flex';
                ...
                const playerOverlayControls = document.querySelector('.player-overlay-controls');
                if (playerOverlayControls) playerOverlayControls.classList.remove('movie-server-top');
            } else {
                if (overlayEpisodeSelector) overlayEpisodeSelector.style.display = 'none';
                // Move server switcher to top left for movies
                const playerOverlayControls = document.querySelector('.player-overlay-controls');
                if (playerOverlayControls) playerOverlayControls.classList.add('movie-server-top');
            }
        }
        ...
        if (!document.getElementById('meta-loading')) {
            ...
            if (metadataArea) {
                ...
                metadataArea.appendChild(spinner);
            }
        }
        ...
        async function fetchWithFallback(endpoint, lang) {
            ...
            if (!data) {
                const resp = await fetch(`${TMDB_BASE_URL}${endpoint}?language=${lang}`);
            ...
            if (lang !== 'en-US') {
                ...
                if (missing.length) {
                    ...
                    if (!enData) {
                        const enResp = await fetch(`${TMDB_BASE_URL}${endpoint}?language=en-US`);
        ...
        window['__onGCastApiAvailable'] = function(isAvailable) {
          if (isAvailable) {
            ...
            const castButtonContainer = document.getElementById('castButtonContainer');
            const castFallback = document.getElementById('castFallback');
            if (castButtonContainer) castButtonContainer.style.display = 'inline-block';
            if (castFallback) castFallback.style.display = 'none';
          } else {
            const castButtonContainer = document.getElementById('castButtonContainer');
            const castFallback = document.getElementById('castFallback');
            if (castButtonContainer) castButtonContainer.style.display = 'none';
            if (castFallback) castFallback.style.display = 'inline';
          }
        };
    </script>
    <!-- Server Error Modal -->
    ...
    <script>
      (function() {
        ...
        const mouseCatcher = document.getElementById('mouse-catcher');
        let hideTimeout;

        function showOverlays() {
          overlays.forEach(el => el.classList.remove('hidden'));
          if (mouseCatcher) mouseCatcher.style.display = 'none'; // Hide catcher to allow iframe interaction
        }
        function hideOverlays() {
          overlays.forEach(el => el.classList.add('hidden'));
          if (mouseCatcher) mouseCatcher.style.display = 'block'; // Show catcher to capture next interaction
        }
        ...
        if (mouseCatcher) {
            mouseCatcher.addEventListener('mousemove', (e) => {
                e.stopPropagation(); // Stop bubbling if needed, but main player listener handles it too
                showOverlays();
                clearTimeout(hideTimeout);
                hideTimeout = setTimeout(hideOverlays, 2000);
            });
            mouseCatcher.addEventListener('click', (e) => {
                e.stopPropagation();
                showOverlays();
                clearTimeout(hideTimeout);
                hideTimeout = setTimeout(hideOverlays, 2000);
